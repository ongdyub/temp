                ...
                for measure_start_tick in range(
                    ...
                    if measure_start_tick + int(ticks_per_measure) > next_tick:
                        measure_events.append(
                            [measure_start_tick, "BOM", numerator, denominator, None, None, 0]
                        )
                        measure_events.append(
                            [next_tick, "EOM", numerator, denominator, None, None, 0]
                        )
                    else:
                        measure_events.append(
                            [measure_start_tick, "BOM", numerator, denominator, None, None, 0]
...
def event_seq_to_str(new_event_seq):
    ...
    for evt in new_event_seq:
        if evt[1] == "ON":
            ...
            char_events.append("N"+str(evt[2]))
            char_events.append("D"+str(evt[3]))
            char_events.append("YZ")
        elif evt[1] == "CC":
...
def mp_handler(file_paths):
    ...
    # ----------------------------------------------------------------------------------
    txt_start = time.time()
    if not os.path.exists('/workspace/QAGen/data/corpus/'):
        os.makedirs('/workspace/QAGen/data/corpus/')

    with open("/workspace/QAGen/data/corpus/raw_corpus.txt", "w", encoding="utf-8") as f:
